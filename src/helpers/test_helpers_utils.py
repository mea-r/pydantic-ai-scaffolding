...
def test_hello_world(model_name: str = 'mistralai/ministral-3b', provider='open_router') -> Tuple[T, LLMReport]:
    base = AiHelper()
    ...
    return base.get_result(prompt, Hello_worldModel, llm_model_name=model_name, provider=provider)
...
def test_weather(model_name: str = 'openai/gpt-4.1', provider='openai') -> Tuple[T, LLMReport]:
    ...
    return base.get_result(prompt, WeatherModel, llm_model_name=model_name, provider=provider, tools=tools)


def test_file_analysis(model_name: str = 'openai/gpt-4o', provider='openai') -> Tuple[T, LLMReport]:
    base = AiHelper()
    ...
    return base.get_result(prompt, FileAnalysisModel, llm_model_name=model_name, provider=provider, file=file_path)
